...
type Props = {
  image: string; // Updated type for `image` to ensure it's a string (URL or static import)
  projectName: string;
  details: string;
  className?: string;
...
const WorksCard: React.FC<Props> = ({
  ...
  return (
    <div className="bg-[#E4E4E4] p-3 md:p-8 rounded-[32px] flex items-center max-w-[582px] flex-wrap mx-auto">
      
      <Image src={image} alt={`${projectName} preview`}/>
      <motion.div
        whileHover={{
          scale: 1.1,
      ...
      >
        <div className="bg-[#CDD7B9] flex flex-col rounded-[20px] gap-6 py-7 px-8">
          <span className="flex justify-between items-center">
            <h5 className="text-[#1C1C1C] text-xl font-bold my-4">{projectName}</h5>
            <div>
              <ForwardArrow />
            </div>
          </span>
